    ...
    <div id="content">
        <div class="outer">
            <div class="inner bg-light no-padding">
                <h3><i class="fa fa-plus"></i> {{ $title ?? 'Add new region'}}</h3>
                <div>
                                    ...
                                    <form action="{{ route('region.store') }}" method="POST">
                                        ...
                                        <div class="form-group @error('country_id') has-error @enderror">
                                            ...
                                            <select name="time_zone_id"
                                                ...
                                                @if($timezone)
                                                    <option value="{{ $timezone->id }}" @if(old('time_zone_id') == $timezone->id) selected @endif>{{ $timezone->name }}</option>
                                                @endif
                                            </select>
                                            @error('time_zone_id')
                                            <div class="help-block with-errors text-danger"><i
                                        ...
                                        <div class="form-group @error('code') has-error @enderror">
                                            ...
                                            <input type="text" name="code" value="{{ old('code') }}" class="form-control" placeholder="Value"
                                                      required>
                                            @error('code')
                                            <div class="help-block with-errors text-danger"><i
                                                    class="fa fa-times"></i> {{ $message }}</div>

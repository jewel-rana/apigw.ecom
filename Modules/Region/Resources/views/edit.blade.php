    ...
    <div id="content">
        <div class="outer">
            <div class="inner bg-light no-padding">
                <h3><i class="fa fa-plus"></i> {{ $title ?? 'Update region'}}</h3>
                <div>
                                    ...
                                    <form action="{{ route('region.update', $region->id) }}" method="POST">
                                        ...
                                        <div class="form-group @error('country_id') has-error @enderror">
                                            ...
                                            <select name="country_id"
                                                ...
                                                @foreach($countries as $country)
                                                    <option value="{{ $country->id }}" @if(old('country_id', $region->country_id) == $country->id) selected @endif>{{ $country->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('country_id')
                                        ...
                                        <div class="form-group @error('country_id') has-error @enderror">
                                            ...
                                            <select name="time_zone_id"
                                                ...
                                                @if($timezone)
                                                    <option value="{{ $timezone->id }}" @if(old('time_zone_id', $region->time_zone_id) == $timezone->id) selected @endif>{{ $timezone->name }}</option>
                                                @endif
                                            </select>
                                            @error('time_zone_id')
                                            <div class="help-block with-errors text-danger"><i
                                        ...
                                        <div class="form-group @error('key') has-error @enderror">
                                            ...
                                            <input type="text" name="name" value="{{ old('name', $region->name) }}"
                                                   class="form-control form-control-lg" id="" placeholder="Name of region"
                                                   required>
                                            @error('name')
                                            <div class="help-block with-errors text-danger"><i
                                                    class="fa fa-times"></i> {{ $message }}</div>
                                        ...
                                        <div class="form-group @error('code') has-error @enderror">
                                            ...
                                            <input type="text" name="code" value="{{ old('code', $region->code) }}" class="form-control" placeholder="Value"
                                                   required>
                                            @error('code')
                                            <div class="help-block with-errors text-danger"><i
                                                    class="fa fa-times"></i> {{ $message }}</div>

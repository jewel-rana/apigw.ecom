    ...
    <div id="content">
        <div class="outer">
            <div class="inner bg-light no-padding">
                <h3><i class="fa fa-plus"></i> {{ $title ?? 'Update supplier'}}</h3>
                <div>
                                ...
                                <div class="col-lg-7">
                                    ...
                                    <form action="{{ route('provider.update', $provider->id) }}" method="POST">
                                        ...
                                        <div class="form-group @error('name') has-error @enderror">
                                            ...
                                            <select class="form-control select2 select2-blue" name="gateway_ids[]" multiple="multiple" data-placeholder="Select gateway" required>
                                                @foreach(app(\Modules\Gateway\GatewayService::class)->all() as $gateway)
                                                    <option value="{{ $gateway->id }}" @if(in_array($gateway->id, old('gateway_ids', $provider->gateway_ids) ?? [])) selected @endif>{{ $gateway->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('name')
                                        ...
                                        <div class="form-group @error('name') has-error @enderror">
                                            ...
                                            <input type="text" name="email" value="{{ old('email', $provider->email) }}"
                                                   class="form-control form-control-lg" id="" placeholder="Email" required>
                                            @error('email')
                                            <div class="help-block with-errors text-danger"><i
                                                    class="fa fa-times"></i> {{ $message }}</div>
                                            @enderror
                                        ...
                                        <div class="form-group @error('password') has-error @enderror">
                                            ...
                                            <input type="password" name="password_confirm" value="{{ old('password_confirm') }}"
                                                   class="form-control form-control-lg" id="" placeholder="Password confirm"
                                                   disabled>
                                        ...
                                        <div class="form-group @error('status') has-error @enderror">
                                            ...
                                            <select name="status" class="form-control">
                                                <option value="1" @if(old('status', $provider->status) == 1) selected @endif>Active
                                                </option>
                                                <option value="0" @if(old('status', $provider->status) == 0) selected @endif>Inactive
                                                </option>
                                            </select>
                                            @error('status')
                                            <div class="help-block with-errors text-danger"><i
